...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		...
		if(p instanceof King && target.getColumn() == source.getColumn() +2) {
			Position st = new Position(source.getRow(), source.getColumn() +3);
			Position tt = new Position(source.getRow(), source.getColumn() +1);
		...
		if(p instanceof King && target.getColumn() == source.getColumn() -2) {
			Position st = new Position(source.getRow(), source.getColumn() -4);
			Position tt = new Position(source.getRow(), source.getColumn() -1);
			ChessPiece rook = (ChessPiece)board.removePiece(st);
			board.placePiece(rook, tt);
			rook.increaseMoveCount();
